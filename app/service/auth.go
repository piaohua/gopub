...
// 用户注册成为代理
func (this *AuthService) Regist(username, password, agent, phone, weixin, qq, address string) error {
	user, err := UserService.GetUserByName(username)
	if err == nil || user.Id != "" {
	...
	user, err = UserService.AddAgencyUser(username, password, agent, phone, weixin, qq, address)
	if err != nil {
		return err
	}
	return nil
